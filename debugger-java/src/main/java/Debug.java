...
public class Debug extends EventHandlerBase {
    ...
    // Better encapsulated state object
    ...
    private final String classPaths;
        // include list of excludes to add

    // Observer pattern? - observers subscribe instead of be passed in
    ...
    public Debug(DebugResponseQueue outQueue,
        ...
        this.state = DeevaState.State.NO_INFERIOR;
    ...
    public Debug(DeevaConfig setup, DebugResponseQueue outQueue, DeevaEventDispatcher dispatcher) {
    ...
    public void start(String programName, List<String> programArgs,
        ...
        EventThread eventThread = new EventThread(vm, this);
        eventThread.start();
        redirectOutput();

        state = DeevaState.State.STASIS;
    ...
    public void putStdInMessage(String msg) throws
        ...
        if (state == DeevaState.State.AWAITING_IO) {
            /* Is this the behaviour that we want? */
            state = DeevaState.State.RUNNING;
        }

        DeevaStateBuilder dsb = new DeevaStateBuilder();
        dsb.setState(state);
        DeevaState state = dsb.build();
    }
    ...
    public void sendState() {
        ...
        DeevaState deevaState = dsb.build();
    ...
    public void stepInto() throws InterruptedException {
        if (state != DeevaState.State.STASIS) {
    ...
    private BreakpointRequest attemptToSetBreakpoint(String clas, int lineNum) throws
            ...
            NoLocationException
    {
        if (state == DeevaState.State.NO_INFERIOR) { throw new NoVMException(); }
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (bytesAvailable == 0) {
                            ...
                            state = DeevaState.State.AWAITING_IO;
    ...
    private void cleanUp() {
        ...
        state = DeevaState.State.NO_INFERIOR;
        sendState();
    }
    ...
    // TODO: move to a DeevaUtils class or something
    ...
    private void redirectOutput() {
        ...
        ioService.setupIO(inputStream, errorStream, outputStream);
         */
    }
